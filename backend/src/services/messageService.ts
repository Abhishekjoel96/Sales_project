...
import { getLeadById, updateLead, getLeadByPhoneNumber, getLeadByEmail } from '../models/Lead'; // Corrected import
...
export const sendMessage = async (leadId: string, channel: 'WhatsApp' | 'SMS' | 'Email', messageContent: string): Promise<Message> => {
    ...
    let messageResponse: any; // To store the Twilio/Mailgun response
    try {
        switch (channel) {
            case 'WhatsApp':
...
export const receiveMessage = async (leadId: string, channel: 'WhatsApp' | 'SMS' | 'Email', messageContent: string, from: string = ''): Promise<Message> => {
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
        { role: "system", content: aiSettings.context || `You are a helpful assistant for BusinessOn.ai.  Your goal is to help qualify leads and schedule appointments. Be ${aiSettings.tone} and ${aiSettings.style}.` },
    ...
    if (aiResponse.toLowerCase().includes('book an appointment')) {
        const appointmentRegex = /(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})/;
    ...
    if (!aiResponse.toLowerCase().includes('book an appointment')) {
        setTimeout(async () => {
            const latestMessages = await getMessagesByChannelAndLeadId(leadId, channel);
